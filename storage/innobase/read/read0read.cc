...
void
ReadView::prepare(trx_t *caller_trx)
{
	ut_ad(mutex_own(&trx_sys->mutex));

	m_creator_trx_id = caller_trx ? caller_trx->id : 0;

	trx_sys->snapshot_ids(caller_trx, &m_ids, &m_low_limit_id);
...
ReadView::copy_complete()
{
	...
	if (m_creator_trx_id > 0) {
		m_ids.insert(std::upper_bound(m_ids.begin(),
					      m_ids.end(),
					      m_creator_trx_id),
			     m_creator_trx_id);
	}
