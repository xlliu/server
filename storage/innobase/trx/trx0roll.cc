                                               ...
                                               trx_ids_t *ids)
{
  ...
  if (element->trx)
  {
    mutex_enter(&element->trx->mutex);
    assert_trx_in_rw_list(element->trx);
    if (element->trx->is_recovered &&
        trx_state_eq(element->trx, TRX_STATE_ACTIVE))
      ids->push_back(element->id);
    mutex_exit(&element->trx->mutex);
  }
  mutex_exit(&element->mutex);
  return 0;
}
...
void trx_rollback_recovered(ibool all)
{
  trx_ids_t ids;
			...
			os_thread_create */
{
	...
	if (trx_sys->rw_trx_hash.size()) {
		ib::info() << "Starting in background the rollback of recovered"
			" transactions";
