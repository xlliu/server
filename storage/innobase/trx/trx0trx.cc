	...
	bool	read_write)	/*!< in: true if read-write transaction */
{
	...
	we don't need trx_sys->mutex protection for that purpose. Still this trx
	can be found through trx_sys->mysql_trx_list, which means state change
	must be protected by e.g. trx->mutex.

	For now we update it without mutex protection, because original code did
	it this way. It has to be reviewed and fixed properly. */
	trx->state = TRX_STATE_ACTIVE;
	...
	if (!trx->read_only
		...
		/* Only trx object owner thread or concurrent thread iterating
		rw_trx_hash and holding trx->element->mutex is allowed to access
		trx->in_rw_trx_list.

		No extra protection is required if these conditions are met. We
		just need to make sure it is updated when trx object is not in
		rw_trx_hash. */
		ut_d(trx->in_rw_trx_list = true);
  ...
  trx_recover_for_mysql_callback_arg *arg)
{
  ...
  if (element->trx)
  {
    ...
    /*
      ...
      PREPARED while we are holding the element->mutex. But since it is executed
      at startup no state change should occur.
    */
    if (trx_state_eq(element->trx, TRX_STATE_PREPARED))
    {
      if (arg->count == 0)
        ib::info() << "Starting recovery for XA transactions...";
      ib::info() << "Transaction " << trx_get_id_for_print(element->trx)
                 << " in prepared state after recovery";
      ib::info() << "Transaction contains changes to " << element->trx->undo_no
                 << " rows";
      arg->xid_list[arg->count++]= *element->trx->xid;
    }
  }
...
int trx_recover_for_mysql(XID *xid_list, uint len)
{
  ...
  if (arg.count)
    ib::info() << arg.count << " transactions in prepared state after recovery";
  return(arg.count);
}
  ...
  trx_get_trx_by_xid_callback_arg *arg)
{
  ...
  if (element->trx)
  {
    ...
    if (element->trx->is_recovered &&
        trx_state_eq(element->trx, TRX_STATE_PREPARED) &&
        arg->xid->eq(reinterpret_cast<XID*>(element->trx->xid)))
    {
      /* Invalidate the XID, so that subsequent calls will not find it. */
      element->trx->xid->null();
      arg->trx= element->trx;
      mutex_exit(&element->mutex);
      return 1;
    }
