...
private:
  /**
    ...
    is 0. We can change the state to COMMITTED_IN_MEMORY to signify that it is
    no longer "active".
  */
...
public:
	...
	* ... -> ACTIVE -> PREPARED (connected) -> PREPARED (disconnected)
	...
	bool		in_rw_trx_list;	/*!< true if in trx_sys->rw_trx_hash */
	/* @} */
#endif /* UNIV_DEBUG */
	UT_LIST_NODE_T(trx_t)
  ...
  void reference()
  {
    ut_ad(my_atomic_load32_explicit(&n_ref, MY_MEMORY_ORDER_RELAXED) >= 0);
