...
restart:
	...
	while (const lock_t *lock = iter.current(trx)) {
		...
		if (lock_get_type_low(lock) == LOCK_REC) {
			...
			if (load_block) {
				...
				release lock mutex if it does a read
				from disk. */
  ...
  lock_rec_other_trx_holds_expl_arg *arg)
{
  ...
  if (element->trx)
  {
    ...
    /*
      ...
      lock.
    */
    ut_a(!expl_lock || expl_lock->trx == arg->impl_trx);
  }
  mutex_exit(&element->mutex);
  return 0;
}
...
static void lock_rec_other_trx_holds_expl(trx_t *trx, const rec_t *rec,
                                          const buf_block_t *block)
{
  if (trx)
  {
	...
	const ulint*		offsets)/*!< in: rec_get_offsets(rec, index) */
{
	...
	if (dict_index_is_clust(index)) {
		...
		ut_d(lock_rec_other_trx_holds_expl(trx, rec, block));
	}
